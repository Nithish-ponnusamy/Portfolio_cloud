import React from "react";
import { motion } from "framer-motion";

const Footer = () => {
  const currentYear = new Date().getFullYear();

  return (
    <footer className="bg-gray-900 py-16 border-t border-gray-800">
      <div className="container mx-auto px-4">
        {/* Centered Content */}
        <motion.div
          className="text-center space-y-8"
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6 }}
        >
          {/* Name and Title */}
          <motion.div
            className="space-y-4"
            initial={{ opacity: 0, scale: 0.9 }}
            whileInView={{ opacity: 1, scale: 1 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6, delay: 0.2 }}
          >
            <h3 className="text-3xl md:text-4xl font-bold text-white">
              Nithish P
            </h3>
            <p className="text-xl text-gray-300 max-w-2xl mx-auto">
              Full Stack Developer & DevOps Engineer
            </p>
            <p className="text-gray-400 max-w-3xl mx-auto leading-relaxed">
              Passionate about creating scalable web solutions and automated
              deployment pipelines using modern technologies like React,
              Node.js, Jenkins, Docker, and Kubernetes.
            </p>
          </motion.div>

          {/* Contact Information */}
          <motion.div
            className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl mx-auto"
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6, delay: 0.6 }}
          >
            {/* Email */}
            <motion.div className="space-y-2" whileHover={{ scale: 1.05 }}>
              <div className="flex items-center justify-center gap-2 text-primary-400">
                <span className="text-xl">📧</span>
                <h4 className="font-semibold">Email</h4>
              </div>
              <p className="text-gray-300">nithinithish271@gmail.com</p>
            </motion.div>

            {/* Phone */}
            <motion.div className="space-y-2" whileHover={{ scale: 1.05 }}>
              <div className="flex items-center justify-center gap-2 text-secondary-400">
                <span className="text-xl">📱</span>
                <h4 className="font-semibold">Phone</h4>
              </div>
              <p className="text-gray-300">+91 6374976526</p>
            </motion.div>

            {/* Location */}
            <motion.div className="space-y-2" whileHover={{ scale: 1.05 }}>
              <div className="flex items-center justify-center gap-2 text-orange-400">
                <span className="text-xl">📍</span>
                <h4 className="font-semibold">Location</h4>
              </div>
              <p className="text-gray-300">Karur, Tamil Nadu</p>
            </motion.div>
          </motion.div>

          {/* Social Links - FIXED SYNTAX */}
          <motion.div
            className="flex justify-center gap-6"
            initial={{ opacity: 0 }}
            whileInView={{ opacity: 1 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6, delay: 0.8 }}
          >
            {[
              {
                name: "LinkedIn",
                url: "#",
                color: "hover:text-blue-400",
                icon: (
                  <svg
                    className="w-6 h-6"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                  </svg>
                ),
              },
              {
                name: "GitHub",
                url: "#",
                color: "hover:text-gray-300",
                icon: (
                  <svg
                    className="w-6 h-6"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
                  </svg>
                ),
              },
              {
                name: "Portfolio",
                url: "#",
                color: "hover:text-primary-400",
                icon: (
                  <svg
                    className="w-6 h-6"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
                      fillRule="evenodd"
                      d="M6 6V5a3 3 0 013-3h2a3 3 0 013 3v1h2a2 2 0 012 2v3.57A22.952 22.952 0 0110 13a22.95 22.95 0 01-8-1.43V8a2 2 0 012-2h2zm2-1a1 1 0 011-1h2a1 1 0 011 1v1H8V5zm1 5a1 1 0 011-1h.01a1 1 0 110 2H10a1 1 0 01-1-1z"
                      clipRule="evenodd"
                    />
                    <path d="M2 13.692V16a2 2 0 002 2h12a2 2 0 002-2v-2.308A24.974 24.974 0 0710 15c-2.796 0-5.487-.46-8-1.308z" />
                  </svg>
                ),
              },
              {
                name: "Email",
                url: "mailto:nithinithish271@gmail.com",
                color: "hover:text-green-400",
                icon: (
                  <svg
                    className="w-6 h-6"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path d="M2.003 5.884L10 9.882l7.997-3.998A2 2 0 0016 4H4a2 2 0 00-1.997 1.884z" />
                    <path d="M18 8.118l-8 4-8-4V14a2 2 0 002 2h12a2 2 0 002-2V8.118z" />
                  </svg>
                ),
              },
            ].map((social, index) => (
              <motion.a
                key={index}
                href={social.url}
                className={`p-3 rounded-full bg-gray-800/50 border border-gray-700/50 text-gray-400 ${social.color} transition-all duration-300`}
                whileHover={{ scale: 1.2, rotate: 5 }}
                whileTap={{ scale: 0.9 }}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 0.3, delay: index * 0.1 + 0.6 }}
                title={social.name}
              >
                {social.icon}
              </motion.a>
            ))}
          </motion.div>

          {/* Navigation Links */}
          <motion.div
            className="flex justify-center gap-8 flex-wrap"
            initial={{ opacity: 0 }}
            whileInView={{ opacity: 1 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6, delay: 1.0 }}
          >
            {["About", "Skills", "Projects", "Contact"].map((link, index) => (
              <motion.a
                key={index}
                href={`#${link.toLowerCase()}`}
                className="text-gray-400 hover:text-primary-400 transition-colors font-medium"
                whileHover={{ scale: 1.1, y: -2 }}
              >
                {link}
              </motion.a>
            ))}
          </motion.div>
        </motion.div>

        {/* Bottom Section */}
        <motion.div
          className="pt-8 mt-8 border-t border-gray-800 text-center"
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6, delay: 1.2 }}
        >
          <div className="flex flex-col md:flex-row justify-center items-center gap-4 text-gray-400">
            <p>
              © {currentYear}{" "}
              <span className="text-white font-semibold">Nithish P</span>. All
              rights reserved.
            </p>
           
          </div>
        </motion.div>
      </div>
    </footer>
  );
};

export default Footer;
